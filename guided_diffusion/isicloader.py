...
class ISICDataset(Dataset):
    def __init__(self, args, data_path , transform = None, mode = 'Training',plane = False):
        ...
        self.transform = transform
        print("len(self.name_list)",len(self.name_list))
    ...
    def __getitem__(self, index):
        """Get the images"""
        name = self.name_list[index]
        img_path = os.path.join(self.data_path, name)#[-17]+".png")
        
        mask_name = self.label_list[index]
        msk_path = os.path.join(self.data_path, mask_name)#[-17]+".png")
