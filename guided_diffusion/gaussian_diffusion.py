...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        self, model, x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None,y=None
    ):
        """
        ...
        cal = 0
        assert t.shape == (B,)
        model_output = model(x, self._scale_timesteps(t),y=y, **model_kwargs)
    ...
    def training_losses_segmentation(self, model, classifier, x_start, t, model_kwargs=None, noise=None,y=None):
        """
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.BCE_DICE or self.loss_type == LossType.RESCALED_MSE:
            ...
            model_output, cal = model(x_t, self._scale_timesteps(t) , **model_kwargs,y=y)#, **model_kwargs)#,y=y)
