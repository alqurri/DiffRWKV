    ...
    def save(self):
        def save_checkpoint(rate, params):
            ...
            if dist.get_rank() == 0:
                ...
                if not rate:
                    filename = f"/scratch/aqa6122/tmp_out/savedmodel{(self.step+self.resume_step):06d}.pt"
                else:
                    filename = f"/scratch/aqa6122/tmp_out/emasavedmodel_{rate}_{(self.step+self.resume_step):06d}.pt"
        ...
        if dist.get_rank() == 0:
            with bf.BlobFile(
                bf.join(get_blob_logdir(), f"/scratch/aqa6122/tmp_out/optsavedmodel{(self.step+self.resume_step):06d}.pt"),
                "wb",
            ) as f:
